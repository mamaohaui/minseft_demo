...
// opts.silent = true 时不弹错误 toast（供带降级逻辑的调用方自行处理失败）
function callCloud(name, data = {}, opts = {}) {
  return new Promise((resolve) => {
    wx.cloud.callFunction({ name, data })
      .then(res => {
        const r = res.result || {}
        if (!opts.silent && r.ok === false && r.message) {
      ...
      .catch(err => {
        ...
        if (opts.silent) {
          return resolve({ ok: false, code: notFound ? 'NOT_DEPLOYED' : 'NETWORK', message: '网络异常' })
        }
        wx.showToast({
